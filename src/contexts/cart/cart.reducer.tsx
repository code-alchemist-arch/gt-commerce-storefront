...
export const cartItemsTotalPrice = (items: Product[], coupon = null) => {
  if (items === null || items.length === 0) return 0;
  const itemCost = items.reduce((total, item) => {
    if (item.variants[0]?.pricing?.price?.gross?.amount) {
      return (
        total + item.variants[0]?.pricing?.price?.gross?.amount * item.quantity
      );
    }
    return (
      total +
      item.variants[0]?.pricing?.priceUndiscounted?.gross?.amount *
        item.quantity
    );
  }, 0);

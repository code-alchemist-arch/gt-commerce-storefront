...
export const Tag = styled.div<{ position: string }>`
  font-size: 12px;
  font-weight: 500;
  color: #ffffff;
  ...
  border-radius: ${themeGet("borderRadius")} 0px;
  width: auto;
  opacity: 0.9;
  z-index: 1;
  &.disabled {
  ...
  &.right {
    border-radius: 0px ${themeGet("borderRadius")};
  }
`;

export const Favourite = styled.span<{ tagPosition: string }>`
  z-index: 1;
  position: absolute;
  ${({ tagPosition }) =>
    tagPosition === "right" ? "left: 20px" : "right: 20px"};
  top: 15px;
...
export const ProductCardPriceBox = styled.div<{ condensed?: boolean }>`
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: space-between;
